...
describe('ScoreBoard', () => {
  it('dispatches score taps for both sides', () => {
    ...
    render(<ScoreBoard state={state} dispatch={dispatch} tx={{ setPoint: 'SET POINT', red: 'VERMELHO', blue: 'AZUL' }} />)

    fireEvent.pointerDown(screen.getByRole('button', { name: /VERMELHO: 0/ }))
    fireEvent.pointerDown(screen.getByRole('button', { name: /AZUL: 0/ }))
  ...
  it('shows set-point labels only when legal next-point win', () => {
    ...
    const { rerender } = render(
      <ScoreBoard
        state={{ ...base, target: 12, scores: { red: 11, blue: 10 } }}
        dispatch={dispatch}
        tx={{ setPoint: 'SET POINT', red: 'VERMELHO', blue: 'AZUL' }}
      />,
    )
